...
from coffeebreak.utils.api import Router, Depends
...
async def test_point_system_service():
    ...
    logger.debug(
        "Testing PointSystemService.leaderboard.get_activity() with activity_id=1"
    )
    ...
    logger.debug(
        "Testing PointSystemService.points.get_in_activity() with activity_id=1 and user_id=1"
    )
    logger.debug(service.points.get_in_activity(1, 1))

    logger.debug(
        "Testing PointSystemService.points.remove() with user_id=1 and points=10"
    )
    logger.debug(service.points.remove("1", 10, "Test removal"))

    logger.debug("Testing PointSystemService.points.get_history() with user_id=1")
    logger.debug(service.points.get_history(1))

    tr = TransactionRequest(activity_id=None, points=10, description="Test transaction")
    logger.debug(
        f"Testing PointSystemService.points.create_transaction() with TransactionRequest {tr} and user_id=1"
    )
